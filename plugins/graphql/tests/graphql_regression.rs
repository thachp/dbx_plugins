...
use graphql_api::{run as run_graphql, Options as GraphqlOptions};
use reqwest::Client;
...
async fn graphql_append_and_query_flow() -> TestResult<()> {
    ...
    let graphql_bind = format!("127.0.0.1:{plugin_port}");
    let plugin_handle = spawn_graphql_plugin(GraphqlOptions {
        bind: graphql_bind.clone(),
        control_addr,
        page_size: config.list_page_size,
        page_limit: config.page_limit,
    })?;

    let base_url = format!("http://{}", graphql_bind);
    ...
    let initial_merkle_root = append_event["merkleRoot"]
        .as_str()
...
fn spawn_graphql_plugin(options: GraphqlOptions) -> TestResult<JoinHandle<anyhow::Result<()>>> {
    Ok(tokio::spawn(async move { run_graphql(options).await }))
}
