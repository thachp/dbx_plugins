...
fn main() -> Result<()> {
    ...
    match mode {
        Mode::Demo => {
            println!("Running in demo mode; emitting a sample event.");
            let bytes = sample_envelope()?;
            listen_and_print(Cursor::new(bytes))
        }
        Mode::Socket(addr) => {
            println!("Connecting to EventDBX socket at {}", addr);
            let stream = TcpStream::connect(&addr)
                .with_context(|| format!("failed to connect to {}", addr))?;
            listen_and_print(stream)
        }
        Mode::Stdin => {
            println!("Reading Cap'n Proto envelopes from stdin…");
...
fn handle_envelope(envelope: plugin_capnp::plugin_envelope::Reader<'_>) -> Result<()> {
        ...
        .context("failed to resolve message union")?
    {
        ...
        plugin_capnp::plugin_envelope::message::Ack(ack) => {
            ...
            println!("[ACK] sequence={}", ack.get_sequence());
        }
